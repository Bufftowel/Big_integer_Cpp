...
class BigInt {
    ...
    BigInt operator + (const BigInt &b) const
    {
        ...
        if(carry) ans.a.push_back(carry);
        return ans;
    }
    template <typename T>
  ...
  //    (you can also define them globally.)
    ...
    friend ostream& operator << (ostream &stream, BigInt num)
    {
        ...
        if(num.a.empty())
            stream << '0';
        else stream << num.a[num.a.size()-1];
    ...
    static vector<ll> karatsuba(vector<ll> a,vector<ll> b)
    {
        ...
        {
            if(i < (int)x2y2.size()) ans[i] += x2y2[i];
            else
            {
                ans[i] += carry;
                if(!carry) break;
        ...
        for(int i = 0; i<(int)ans.size(); ++i)
        {
            if(i<(int)x1y1.size()) ans[i] += x1y1[i];
            else
            {
                ans[i] += carry;
                if(!carry) break;
